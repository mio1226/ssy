...
CREATE TABLE IF NOT EXISTS hard_disk (
    id BIGINT AUTO_INCREMENT PRIMARY KEY,
    model VARCHAR(100) NOT NULL COMMENT '硬盘型号(如捷移,西数SSD等)',
    sn VARCHAR(200) NOT NULL COMMENT 'SN码',
    capacity DECIMAL(10,2) NOT NULL COMMENT '磁盘容量(TB)',
    location VARCHAR(255) DEFAULT NULL COMMENT '硬盘位置(具体地点)',
    purchase_time DATETIME DEFAULT NULL COMMENT '采购时间',
    purchase_price DECIMAL(10,2) DEFAULT NULL COMMENT '采购单价(人民币)',
    purchase_oa_no VARCHAR(100) DEFAULT NULL COMMENT '采购OA流程编号',
    remark VARCHAR(500) DEFAULT NULL COMMENT '备注',
    creator_id BIGINT DEFAULT NULL COMMENT '创建人ID',
    is_idle TINYINT(1) DEFAULT 1 COMMENT '是否空闲 0=使用中 1=空闲',
...
CREATE TABLE IF NOT EXISTS disk_usage_record (
    ...
    storage_content TEXT DEFAULT NULL COMMENT '存储信息内容',
    operator_id BIGINT DEFAULT NULL COMMENT '操作人ID',
    parent_record_id BIGINT DEFAULT NULL COMMENT '父记录ID(关联上一条使用记录)',
    create_time DATETIME DEFAULT CURRENT_TIMESTAMP,
    update_time DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    deleted TINYINT DEFAULT 0 COMMENT '逻辑删除'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE IF NOT EXISTS violation_record (
    ...
    type VARCHAR(50) NOT NULL COMMENT '违规类型 timeout/reuse',
...
('普通用户', 'USER', '普通用户，使用硬盘管理功能');

-- 插入默认规则配置
INSERT INTO rule_config (rule_key, rule_value, description, status) VALUES
('timeout_days', '7', '硬盘出库超时天数，超过此天数未入库视为超时', 1),
('feishu_app_id', '', '飞书应用 App ID（用于导出到飞书电子表格）', 1),
('feishu_app_secret', '', '飞书应用 App Secret（用于导出到飞书电子表格）', 1),
('feishu_spreadsheet_token', '', '飞书电子表格 Token（从表格 URL 中获取）', 1);
